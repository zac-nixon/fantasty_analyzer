...
def adjustQB(QBs,medianPass,medianPoint,medianPassAttempts):
    for q in QBs:
        ...
        pointMultiplerFromDef = (float(oppPass) / float(medianPass)) + (float(oppPoints) / float(medianPoint))
        pointMultiplerFromAttempts = (float(q.attempts) / float(medianPassAttempts))
        finalMultipler = (pointMultiplerFromAttempts + pointMultiplerFromDef) / 2
